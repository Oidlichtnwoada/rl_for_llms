...
uv run accelerate launch --num_processes=0 --num_machines=1 --mixed_precision="$MIXED_PRECISION" --dynamo_backend="$DYNAMO_BACKEND" -m rl_for_llms.evaluate 2>&1 | tee -a logs/rl_for_llms.evaluate.log
