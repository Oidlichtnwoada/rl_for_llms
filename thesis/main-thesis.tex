...
% NOTE TO SELF: state clearly that GSM8K is the *only* dataset used for both
% training and evaluation in this thesis. No multi-domain or cross-dataset
% transfer experiments were run; this is a deliberate scoping decision
% driven by the compute budget (single GV100, float32) and is revisited as
% a threat to external validity in sec:disc:threats and as future work in
% ch:conclusion (cf. \citep{mathdataset,deepmath103k,evalllmscode}).
...
% NOTE TO SELF -- training-budget observations to fold into prose:
%   * Epoch count: the default reported configuration trains for 1 epoch over
%     the filtered GSM8K split. A 2-epoch ablation was also executed; the
%     accuracy / MCC deltas were within seed noise (negligible), consistent
%     with the small effective dataset size. Report this explicitly so the
%     reader does not assume undertraining.
%   * Model family: only Qwen2.5-0.5B-Instruct was actually trained. The
%     experimental plan also included the 0.8B variant of the Qwen3.5
%     successor family for a scale ablation, but it was dropped because
%     (i) the larger model would have multiplied GPU-hours per run and
%     (ii) the existing ablation grid (Base / GRPO / Dense $\pm$ reward /
%     LaSeR $\pm$ reward / three inference-time overlays) already saturated
%     the available compute budget on a single GV100. Frame this as a
%     scoping decision, not a methodological gap; revisit as future work
%     in ch:conclusion.
...
% TODO[setup:reproducibility]: Short, explicit statement (~half a page) to
% satisfy the JKU mathesis rubric on scientific honesty / replicability.
% Cover, in this order:
%   - Code: public GitHub repository
%     \url{https://github.com/Oidlichtnwoada/rl_for_llms} (commit hash to be
%     pinned at submission time). Build/lint/test entry points:
%     \texttt{./check\_repo.sh}, \texttt{./run\_repo.sh}, \texttt{./run\_eval.sh}.
%   - Determinism: fixed \texttt{SEED} environment variable threaded through
%     \texttt{environment\_utils.get\_seed}; deterministic CuBLAS/cuDNN.
%   - Configuration: every hyperparameter lives in the central
%     \texttt{Config} (rl\_for\_llms/models/config.py) and is exported in
%     \autoref{app:hyperparams}. Exact \texttt{pyproject.toml} pinned in
%     \autoref{app:pyproject}; code entry points in \autoref{app:code}.
%   - Data: HuggingFace \texttt{gsm8k} with the deterministic preprocessing
%     described in \autoref{sec:setup:dataset}; processed splits cached under
%     \texttt{data/training/}.
%   - Artifacts: raw run logs and aggregated CSVs released under
%     \texttt{data/evaluation/final/}; W\&B run identifiers in the README.
%   - Hardware: NVIDIA Quadro GV100 (Volta, float32). Document the
%     non-CUDA fallback path (\texttt{MODE=LOCAL}) for portability.
% No new citations -- this section is purely procedural.
...
% STRUCTURE: Two top-level sections to make the training-vs-inference axis
% visible in the TOC. Each result block is a \subsection so the reader sees
% the grouping immediately; preliminary blocks are flagged inline rather than
% via separate sections.

\section{Training-Time Results}
\label{sec:res:training}

% TODO[res:training]: One-paragraph preamble: which variants are compared,
% which checkpoint defines "final", and which metric families are reported
% (answer-correctness vs. binary-classification of correctness).
...
% These results are \textbf{final}. Although selection is a post-hoc
% aggregation, it is reported in the training-time section because it does
% not change generation -- only the choice among already-generated samples.

\section{Inference-Time Scaling Results}
\label{sec:res:inference}

% TODO[res:inference]: One-paragraph preamble: all inference-time scaling
% overlays are evaluated on top of the Dense+loss+reward checkpoint; the
% generation distribution itself is modified (tempmod) or trajectories are
% resampled (SMC). All subsections below are flagged \textbf{preliminary}
...
% NOTE TO SELF -- additional limitations to spell out in prose:
%   * Hyperparameter coverage: the presented methods expose a *large* knob
%     set -- confidence-loss weight, class-reweighting factors, confidence-
%     reward blend coefficient, sigmoid scale, LoRA r/alpha, GRPO group size
%     G, KL coefficient, learning rate, tempmod min/max temperature,
%     confidence-history length, tempmod inversion flag, SMC filtering
%     threshold, and (implicit) the SMC particle count G. The available
%     compute did not permit a proper sweep over this joint space; reported
%     values are reasonable defaults guided by RPT/SR experiments rather
%     than an exhaustive search. State plainly that the reported numbers
%     are therefore an under-estimate of what each method can deliver with
%     a tuned configuration.
%   * Single model size (0.5B): trends may or may not survive at 0.8B+ in
%     the Qwen3.5 family; see sec:disc:threats T3.
%   * Single dataset (GSM8K only): no cross-dataset transfer experiments
%     were run; restricts external validity (cf. \citep{mathdataset,
%     deepmath103k,evalllmscode}).
%   * Epoch count: a 2-epoch ablation produced negligible gains on this
%     small split, so longer training was not pursued -- this caps the
%     ceiling reachable on GSM8K alone but does not justify discarding the
%     method.

\section{Threats to Validity}
\label{sec:disc:threats}

% TODO[disc:threats]: JKU mathesis rubric expects an explicit
% threats-to-validity discussion. Organise as four numbered threats, each
% with a mitigation paragraph:
%   T1 (internal validity) -- Single random seed for most variants; risk of
%     mistaking seed noise for method differences. Mitigation: report per-
%     group means and standard deviations, flag preliminary runs (LaSeR,
%     tempmod, SMC, combined) as such, and avoid absolute ranking claims.
%   T2 (construct validity) -- Confidence-loss weighting and the choice of
%     intrinsic reward blend may overfit to the BCE auxiliary signal rather
%     than to true correctness; risk of reward hacking
%     (\citep{rewardoveropt,spuriousrewards}). Mitigation: report MCC /
%     balanced accuracy / confidence-token sampling rate alongside Pass@k,
%     and use the dense Dense+loss-only ablation as a no-reward control.
%   T3 (external validity) -- Single dataset (GSM8K, grade-school math) and
%     a 0.5B-parameter Qwen2.5 model; results may not transfer to harder
%     math (\citep{mathdataset,deepmath103k}), code (\citep{evalllmscode}),
%     or frontier-scale models. Mitigation: scope claims to the experimental
%     regime; defer cross-domain generalisation to future work.
%     Additionally note that a Qwen3.5-0.8B scale ablation was planned but
%     not executed for compute reasons (see sec:setup:model), so model-
%     family scaling behaviour is currently extrapolation, not measurement.
%   T4 (statistical conclusion validity) -- Multiple comparisons across
%     variants without formal multiple-testing correction; reliability
%     metrics (ECE/Brier) reported on a finite evaluation split. Mitigation:
%     use effect-size language ("substantially higher MCC") rather than
%     p-value claims, and release raw per-sample CSVs for re-analysis.
% No new citations beyond those already listed.
...
% System-2 agents.
% NOTE TO SELF -- explicit future-work bullets to add on top of the
% directions already enumerated in RPT \autoref{sec:conclusion}:
%   * Hyperparameter exploration: a proper sweep over the knobs listed in
%     sec:disc:limitations (confidence-loss weight, reward blend, sigmoid
%     scale, LoRA r/alpha, GRPO G, learning rate, tempmod T_min/T_max,
%     inversion flag, confidence-history length, SMC filtering threshold,
%     particle count) is expected to substantially tighten every metric
%     reported here and to clarify which inference-time overlay dominates.
%   * Larger / newer models: re-running the full ablation grid on
%     Qwen3.5-0.8B (the originally planned scale ablation, dropped for
%     compute reasons) and beyond would test whether Dense Confidence's
%     gains persist or grow with model capacity \citep{qwen25}.
%   * More training data: the dense confidence head is supervised only by
%     the broadcast binary correctness signal; with substantially more
%     trajectories -- e.g. moving from GSM8K \citep{gsm8k} to MATH
%     \citep{mathdataset}, DeepMath-103K \citep{deepmath103k} or code
%     benchmarks \citep{evalllmscode} -- the resulting confidence scores
%     are expected to become markedly better calibrated. This would in
%     turn sharpen every downstream consumer of the signal: confidence-
%     guided answer selection \citep{selfconsistency,bonalignmentpolicy,
%     bonselectionselfcertainty,deepthinkwithconfidence,
%     confimprovesselfconsistency}, temperature modulation, and SMC
%     particle filtering. State this expectation explicitly so the reader
%     understands that the inference-time-scaling numbers presented are
%     a *lower bound* on what a better-trained confidence head can deliver.
