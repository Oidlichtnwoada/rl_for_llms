...
The analysis confirmed that the "last-token self-rewarding score" effectively captures solution correctness, enabling the model to act as its own verifier during both training and testing \citep{laser}. The simplification of the reference model probability to a constant was empirically validated, which further reduced computational overhead \citep{laser}. However, the method's generalization to "inexact" domains (such as general reasoning tasks in MMLU-Pro) was observed to be less effective compared to the mathematical domain \citep{laser}.
