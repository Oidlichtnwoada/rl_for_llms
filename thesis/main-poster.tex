...
\documentclass[17pt, a0paper, landscape, margin=14mm, innermargin=13mm,
...
\definecolor{cardframe}{HTML}{C7CFDC} % soft grey card border
...
%% Compact helper for the highlighted headline statistics.
\newcommand{\statbox}[2]{%
	\begin{minipage}[t]{0.185\linewidth}
		\centering
		{\color{jkuaccent}\bfseries\fontsize{40}{44}\selectfont #1}\\[3pt]
		{\footnotesize #2}
	\end{minipage}%
}
...
\settitle{
	\begin{minipage}[c]{0.82\linewidth}
		\raggedright
		{\color{titlefgcolor}\bfseries\fontsize{48}{54}\selectfont \@title\par}
		\vspace{8pt}
		{\color{titlefgcolor}\large \textbf{\@author} \quad\textbar\quad
			Supervisor: \textbf{Sepp Hochreiter} \quad\textbar\quad
			Co-Supervisors: Johannes Schimunek, Christoph Bartmann \quad\textbar\quad
			\@institute\par}
	\end{minipage}\hfill
	\begin{minipage}[c]{0.15\linewidth}
		\raggedleft
		\includegraphics[width=\linewidth]{logos/jku_en_white.pdf}
	\end{minipage}
}
\makeatother
...
\block{}{%
	...
	\centering
	\statbox{$\AccBeforePct{\AnsBase}{pass@1}{0}\!\to\!\AccAfterPct{\AnsDenseSR}{pass@1}{0}$}{\textit{Pass}@1 (\textit{Base} $\to$ \textit{Dense})}\hfill
...
\begin{columns}
	...
	\block{Method: \textit{Dense Confidence} (ours)}{%
		...
		\[
			L_{\mathrm{total}} = -\mathcal{J}_{\mathrm{GRPO}}(\theta) + \beta_{\mathrm{conf}}\,L_{\mathrm{conf}},\qquad
	...
	\block{Selected references}{%
		\small
		[1] Yang et al. \textit{LaSeR: RL with Last-Token Self-Rewarding}, 2025.\;
		[2] Shao et al. \textit{DeepSeekMath} (\acit{GRPO}), 2024.\;
		[3] Cobbe et al. \textit{Training Verifiers to Solve Math Word Problems} (\acit{GSM8K}), 2021.\;
		[4] Hu et al. \textit{LoRA}, 2021.\;
		[5] Qwen Team. \textit{Qwen2.5 Technical Report}, 2025.\;
	...
	\block{Training-time architecture}{%
		\begin{center}
			\resizebox{0.60\linewidth}{!}{\input{architecture_tikz.tex}}
		\end{center}
		\vspace{-4pt}
		{\small The confidence-token logit is harvested \emph{twice} by the same \acit{LM}-head hook: gradient-free
			during generation (intrinsic reward) and with gradients during re-evaluation (auxiliary \acit{BCE}
			loss). The external verifier supplies the outcome bit $r$; the internal verifier supplies $c_{i,t}$.}
	}

	\block{Inference-time scaling (reuses the trained head)}{%
		Three controllers reuse the \textit{Dense} head at evaluation only, with \emph{no} further training:
		\begin{itemize}
		...
		\end{itemize}
		The latter two are \emph{structurally exclusive} to \textit{Dense}: they consume a per-token signal that
		the last-token \textit{LaSeR} parameterization cannot provide.
	}
	...
	\block{Results: training-time}{%
		...
		\begin{itemize}
			...
			\item \textbf{Dense $>$ \textit{LaSeR} (RQ3).} \textit{Dense} sits $2$--$3$ pp above \textit{LaSeR} on every
			      ...
			      advantage (\acit{MCC} $\ConfAfter{\ConfLaserAux}{mcc}\!\to\!\ConfAfter{\ConfLaserSR}{mcc}$), the exact
			      failure the bounded design prevents.
		\end{itemize}
	}

	\block{The per-step signal is an early-warning indicator}{%
		\begin{center}
			\includegraphics[width=0.48\linewidth]{charts/confidence_evolution_chart.pdf}
		\end{center}
		\vspace{-6pt}
		{\small Per-token confidence on a \emph{correct} (higher mean) vs.\ \emph{incorrect} (lower mean)
			\acit{GSM8K} rollout: the trajectories diverge \emph{hundreds of tokens early}, an internally decodable
			signal of eventual success or failure (RQ4).}
	}
	...
	\block{Conclusion \& outlook}{%
		...
		forward pass. The same head decouples two inference-time objectives on one checkpoint: tempmod $+$ \acit{SMC}
		filtering for calibration (\acit{MCC} up to $\ConfAfter{\ConfTempFil}{mcc}$), inverse tempmod for answer
		aggregation. Gains should grow with scale, data, and particle budget $G$; the head is a natural \acit{PRM}
		distillation target. \textbf{Code:} \texttt{github.com/Oidlichtnwoada/rl\_for\_llms}
	}
